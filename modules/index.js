...
function updateActionHistory(gameState, playerId, diceRoll, pieceIndex, move, logger, killedPieceInfo) {
...
function determineNextPlayerId(gameState, currentPlayerId, diceRoll, killedPieceInfo) {
    if (diceRoll === 5) {
...
function getActionsFromIndex(gameState, start) {
    var filteredHistory = {};
...
function rpcClaimDailyReward(ctx, logger, nk, payload) {
    ...
    let rewardData = { lastClaimDate: null, consecutiveDays: 0 };
    const objectId = {
    ...
    if (objects.length > 0 && objects[0].value) {
        rewardData = objects[0].value; // Type assertion
    }
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    let lastClaimDate = rewardData.lastClaimDate ? new Date(rewardData.lastClaimDate) : null;
    ...
    if ((lastClaimDate === null || lastClaimDate === void 0 ? void 0 : lastClaimDate.getTime()) === today.getTime()) {
    ...
    else if ((lastClaimDate === null || lastClaimDate === void 0 ? void 0 : lastClaimDate.getTime()) === yesterday.getTime()) {
        // Consecutive claim
...
function checkDailyRewards(ctx, logger, nk, payload) {
    if (!ctx.userId) {
        throw new Error("No user ID in context");
    }
...
function rpcDeductCoins(context, logger, nk, payload) {
    if (!context.userId) {
        throw Error("No user ID in context");
    }
    var parsedPayload;
...
function getBalance(ctx, logger, nk, payload) {
    if (!ctx.userId) {
        throw new Error("No user ID in context");
    }
    ...
    try {
        ...
        logger.error("storageRead error for user ".concat(ctx.userId, ": ").concat(error));
        return JSON.stringify({
            success: false,
...
function updateBalance(ctx, logger, nk, userId, amount, isDeposit) {
    var _a;
    var objectId = {
        collection: "coins",
        key: "balance",
    ...
    var version; // Version should be undefined initially.
    try {
        var objects = nk.storageRead([objectId]);
        if (objects.length > 0) {
            var storedValue = objects[0].value;
            currentBalance = (_a = storedValue.coins) !== null && _a !== void 0 ? _a : 0;
...
function rpcUpdateBalance(ctx, logger, nk, payload) {
    if (!ctx.userId) {
        throw new Error("No user ID in context");
    }
...
function rpcSavePlayerInfo(context, logger, nk, payload) {
    ...
    var existingPlayerInfo = objects.length > 0 ? objects[0].value : {};
    // Merge existing data with new data
    var updatedPlayerInfo = Object.assign(Object.assign({}, existingPlayerInfo), newPlayerInfo);
...
function rpcGetPlayerInfo(context, logger, nk) {
...
function generateReferralCode(ctx, logger, nk, payload) {
...
function validateReferralCode(ctx, logger, nk, payload) {
    var userId = ctx.userId;
    if (!userId) {
...
function createLeaderboard(ctx, logger, nk, payload) {
    var id = "leaderboard_id";
    var authoritative = true;
    var sort = "descending" /* nkruntime.SortOrder.DESCENDING */; // Use enum instead of string
    var operator = "best" /* nkruntime.Operator.BEST */; // Use enum instead of string
...
function writeScore(ctx, logger, nk, payload) {
    try {
        var request = JSON.parse(payload);
...
function fetchLeaderboard(ctx, logger, nk, payload) {
    try {
        var request = JSON.parse(payload);
        var leaderboardId = request.leaderboardId;
...
function storeWord(context, logger, nk, payload) {
    if (!context.userId) {
        throw Error("No user ID in context");
    }
    var parsedPayload;
    ...
    try {
        ...
        var writeRequest = {
            ...
            permissionRead: 1, // ✅ Use enum
            permissionWrite: 0, // ✅ Use enum
        };
        nk.storageWrite([writeRequest]);
...
function fetchWordsList(context, logger, nk, payload) {
    if (!context.userId) {
        throw Error("No user ID in context");
    }
    var objectId = {
    ...
    try {
        ...
        logger.error("storageRead error for user ".concat(context.userId, ": ").concat(error));
        return JSON.stringify({
            success: false,
